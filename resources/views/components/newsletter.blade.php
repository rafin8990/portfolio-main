<section class="py-20 md:py-32 xl:py-44 flex items-center justify-center text-white ">
    <div class="flex items-center justify-center">
        <div>
            <div
                class="mx-auto text-center text-sm px-4 py-1 border flex gap-2 items-center justify-center border-white w-fit rounded-full">
                <div class="w-2 h-2  md:w-3 md:h-3 animate-pulse transition-all bg-green-500 rounded-full">
                </div>
                <p> Available for work</p>
            </div>
            <div class="text-center mt-10 mx-auto">
                <p class="text-2xl md:text-5xl xl:text-7xl leading-none ">Available
                    for
                    Work
                    Let's collaborate and
                    bring your vision to life!
                </p>

            </div>
            <div
                class="mx-auto text-center px-4 py-2  md:px-6 md:py-2  font-bold border flex gap-2 items-center justify-center bg-white text-black mt-5 w-fit rounded-full">

                <p><a target="blank" href="mailto:someone@example.com">rafin.hossain.8990@gmail.com</a></p>
            </div>
        </div>
    </div>    
</section>